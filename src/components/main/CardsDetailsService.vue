...
const item = servicio.find((h) => h.id === id) || servicio[0];
...
function reserve() {
  // simple reservation flow: navigate to servicio list or to a reservation route
  router.push({ path: "/servicios" });
}

function goBack() {
  // navigate back to servicios listing
  router.push({ path: "/servicios" });
}
</script>
